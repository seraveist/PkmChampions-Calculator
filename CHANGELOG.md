...
- 기술별 피해 영역에 공격측 조건으로 계산한 결정력과 짧은 HP바 추가. 고정피해는 숫자로, 상대 조건이 필요한 값은 `—`로 표시.
- HP바·N타에 타입 무효 적용. 기합의띠·탈의 생존/차단 제외와 먹다남은음식의 N−1회 회복 정책 유지.
- 결정력의 상대 상세 조건 의존성 제거 및 급소 배율 보완. 특성·아이템 배지를 이름으로 정리.
- 메뉴 전환의 일반 안내문구 제거. 메가루카리오Z 파동의방호의 접촉 피해 반감·비접촉 예외 회귀 검사와 설명 보완.
