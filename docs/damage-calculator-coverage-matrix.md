...
| --- | ---: |
| 챔피언스 포켓몬 | 350 |
| 챔피언스 learnset 기술 | 509 |
| 챔피언스 포켓몬이 보유한 특성 | 216 |
| 챔피언스 도구 데이터 | 167 |
| 추적 후보 기술 | 61 |
| 추적 후보 특성 | 76 |
| 추적 후보 도구 | 48 |
...
| 종류 | 그룹 | 항목 | 판정 | 비고 |
| --- | --- | --- | --- | --- |
| field | 더블 아군 보정 | Battery | 지원 근거 없음 | 양측 아군 보정 입력으로 적용 |
| field | 더블 아군 보정 | Friend Guard | 지원 근거 없음 | 양측 아군 보정 입력으로 적용 |
| field | 더블 아군 보정 | Power Spot | 지원 근거 없음 | 양측 아군 보정 입력으로 적용 |
...
| 가변 위력 | 와이드포스 (expandingforce) | 지원 감지 | mechanics, built-data, code | computeVariableBp()에서 처리 |
| 가변 위력 | 객기 (facade) | 지원 감지 | mechanics, built-data | computeVariableBp()에서 처리 |
| 가변 위력 | 변덕레이저 (ficklebeam) | 지원 감지 | mechanics, built-data | 일반·강화 선택 또는 70:30 독립 확률 분포 반영 |
| 가변 위력 | 바둥바둥 (flail) | 지원 감지 | mechanics, built-data | computeVariableBp()에서 처리 |
| 가변 위력 | 내던지기 (fling) | 지원 감지 | mechanics, built-data, code | computeVariableBp()에서 처리 |
...
| 가변 위력 | 탁쳐서떨구기 (knockoff) | 지원 감지 | mechanics, built-data, code | computeVariableBp()에서 처리 |
...
| 가변 위력 | 미스트버스트 (mistyexplosion) | 지원 감지 | mechanics, built-data, code | computeVariableBp()에서 처리 |
...
| 가변 위력 | 토해내기 (spitup) | 지원 감지 | mechanics, built-data | computeVariableBp()에서 처리 |
...
| 고정/비표준 대미지 | 앙갚음 (comeuppance) | 지원 감지 | mechanics, built-data | 받은 HP 피해량 및 물리·특수 입력으로 산출 |
| 고정/비표준 대미지 | 카운터 (counter) | 지원 감지 | mechanics, built-data | 받은 HP 피해량 및 물리·특수 입력으로 산출 |
| 고정/비표준 대미지 | 죽기살기 (endeavor) | 지원 감지 | mechanics, built-data | fixedDamageAmount()에서 처리 |
| 고정/비표준 대미지 | 목숨걸기 (finalgambit) | 지원 감지 | mechanics, built-data, code | fixedDamageAmount()에서 처리 |
...
| 고정/비표준 대미지 | 메탈버스트 (metalburst) | 지원 감지 | mechanics, built-data | 받은 HP 피해량 및 물리·특수 입력으로 산출 |
| 고정/비표준 대미지 | 미러코트 (mirrorcoat) | 지원 감지 | mechanics, built-data | 받은 HP 피해량 및 물리·특수 입력으로 산출 |
...
| 공격/BP/방어 보정 | 풀모피 (grasspelt) | 지원 감지 | mechanics, built-data |  |
...
| 공격/BP/방어 보정 | 강철정신 (steelyspirit) | 지원 감지 | mechanics, built-data |  |
...
| 대미지 보정 | 펑크록 (punkrock) | 지원 감지 | mechanics, built-data |  |
...
| 면역/상성 | 타오르는불꽃 (flashfire) | 지원 감지 | mechanics, built-data, code |  |
| 면역/상성 | 부유 (levitate) | 지원 감지 | mechanics, built-data, code |  |
...
| 위협 차단 | 파수견 (guarddog) | 지원 감지 | mechanics, code | entry-effects.json의 위협 차단 목록으로 적용 전 차단 |
...
| 위협 차단 | 주눅 (rattled) | 지원 감지 | mechanics, code | entry-effects.json의 위협 차단 목록으로 적용 전 차단 |
...
| 자동 진입 효과 | 그래스메이커 (grassysurge) | 지원 감지 | mechanics | entry-effects.json을 makeCalcState()에서 계산용 복사본에 적용 |
| 자동 진입 효과 | 위협 (intimidate) | 지원 감지 | mechanics | entry-effects.json을 makeCalcState()에서 계산용 복사본에 적용 |
| 자동 진입 효과 | 사이코메이커 (psychicsurge) | 지원 감지 | mechanics | entry-effects.json을 makeCalcState()에서 계산용 복사본에 적용 |
| 자동 진입 효과 | 모래뿜기 (sandspit) | 지원 감지 | mechanics, code | entry-effects.json을 makeCalcState()에서 계산용 복사본에 적용 |
...
| 그룹 | 항목 | 판정 | 지원 근거 | 비고 |
| --- | --- | --- | --- | --- |
| KO 추정 | 먹다남은음식 (leftovers) | 지원 감지 | mechanics, built-data | hkoLabel(): 살아남은 뒤 회복, 열매 1회 소비, 독립 난수 합산 |
| KO 추정 | 오랭열매 (oranberry) | 지원 감지 | mechanics, built-data | hkoLabel(): 살아남은 뒤 회복, 열매 1회 소비, 독립 난수 합산 |
| KO 추정 | 자뭉열매 (sitrusberry) | 지원 감지 | mechanics, built-data | hkoLabel(): 살아남은 뒤 회복, 열매 1회 소비, 독립 난수 합산 |
| 가변 위력/접지 보조 | 풍선 (airballoon) | 지원 감지 | mechanics, built-data |  |
| 가변 위력/접지 보조 | 검은철구 (ironball) | 지원 감지 | mechanics, built-data |  |
| 결정력 생존 안내 | 기합의머리띠 (focusband) | 지원 감지 | code | 결과 카드에 생존 효과를 안내하고 순수 피해·KO 산출에서는 제외 |
| 결정력 생존 안내 | 기합의띠 (focussash) | 지원 감지 | mechanics, built-data | 결과 카드에 생존 효과를 안내하고 순수 피해·KO 산출에서는 제외 |
...
| 그룹 | 항목 | 판정 | 지원 근거 | 비고 |
| --- | --- | --- | --- | --- |
| 더블 아군 보정 | Battery | 지원 근거 없음 | - | 양측 아군 보정 입력으로 적용 |
| 더블 아군 보정 | Friend Guard | 지원 근거 없음 | - | 양측 아군 보정 입력으로 적용 |
| 더블 아군 보정 | Power Spot | 지원 근거 없음 | - | 양측 아군 보정 입력으로 적용 |
